...
certificates = '<section class="certifications" aria-label="专业认证"><h2>专业认证</h2>' + ''.join(
    f'<div class="certification-card"><span class="certification-mark" aria-hidden="true">ACP</span><div><h3>{e(c["title"])}</h3><p class="certification-name">{e(c["official_name"])}</p><p class="certification-meta">{e(c["issuer"])} · 有效期至 <time datetime="{e(c["valid_until"],quote=True)}">{e(c["valid_until"])}</time></p><a class="certificate-link" href="{e(c["url"],quote=True)}" target="_blank" rel="noopener noreferrer">查看证书原件 <span aria-hidden="true">↗</span><span class="sr-only">（在新标签页打开）</span></a></div></div>'
